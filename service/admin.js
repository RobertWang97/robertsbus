...
var adminRepo = {
    ...
    //2017-03-28 Robert added router for comment
    'getCommentList': function (fromDate, toDate) {
        var fDate = parseInt(fromDate.replace(/[/-]/g, ""));
        var tDate = parseInt(toDate.replace(/[/-]/g, ""));
        query = {
            "q": "IntDate:[" + fDate + " TO " + tDate + "] AND docType:\"comment\"",
            "sort": "IntDate"
        }
        return mydb.search("Comment", "idx_comment", query, function (er, result) {
            if (result.rows.length !== 0) {
                var r = [];
                result.rows.forEach(function (d) {
                    var doc = d.fields;
...
var adminService = {
    ...
    //2017-03-22 Robert added router for statistics
    'getComment': function (fromDate, toDate) {
        return adminRepo.getCommentList(fromDate, toDate).then(function (data) {
            return data.rows;
        });
    },
    'auditUser': function (username, auditInfo) {
